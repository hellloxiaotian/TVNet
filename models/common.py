...
class DyConv2(nn.Module):
    # Standard convolution
    def __init__(self, c1):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DyConv2, self).__init__()
        k = 3
        act = True
        self.conv = dynamic_conv2d(c1, c1, k, k//2)
...
class DyConv3(nn.Module):
    # Standard convolution
    def __init__(self, c1):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DyConv3, self).__init__()
        k = 1
        act = True
        self.conv = DRConv2d(c1, c1, kernel_size=k, region_num=8)
        self.bn = nn.BatchNorm2d(c1)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
...
class HAB(nn.Module):
    ...
    def forward(self, x, x_size, attn_mask=None):
        h, w = x_size
        b, _, c = x.shape
...
class HABlock(nn.Module):
    ...
    def forward(self, x):
        x_size = (80*2, 80*2)
        # return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params), x_size)))
        return self.patch_unembed(self.hab(self.patch_embed(x), x_size), x_size)
...
# Dynamic convolution
class DyConvolution(nn.Module):
    def __init__(self, in_channels=64, kernel_size=3):
        super(DyConvolution, self).__init__()
        self.kernel_size = kernel_size
